...
RSpec.describe User, :type => :model do
  ...
  describe 'send:notification Task -> Send User Notifications' do
    ...
    before do
      ...
      allow(@gcm).to receive(:send_notification).with(instance_of(Array), instance_of(Hash)).and_return []
    ...
    it 'should send notification to users' do
      ...
      (1..100).each do
        threads << Thread.new do
          user_with_mobile = (1..10).map { FactoryGirl.attributes_for :user_with_mobile }
      ...
      expect(@gcm).to receive(:send_notification).at_least(:once).with(array_length_greater_than_or_eql(1000), instance_of(Hash))
